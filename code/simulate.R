...
sim_gaussian = function(X, pve, effect_num, effects){
  n = dim(X)[1]
  p = dim(X)[2]
...
sim_gaussian_multiple = function(X, pve, effect_num, effects, n_traits=1) {
  meta = list(residual_variance = vector())
  Y = NULL
  for (r in 1:n_traits) {
    res = sim_gaussian(X, pve, effect_num, effects)
