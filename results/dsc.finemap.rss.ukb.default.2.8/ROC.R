...
for (case in 1:nrow(all.comb)){
  ...
  output2_name = paste0('susie_rss_ukb_default_roc/susie_rss_ukb_default_pip_init',initial,'_AZ',add_z,'_ld',ldmethod,'_ER',
                        estimate_resid, '_lamb', lambda, '_roc_2')
  outputall_name = paste0('susie_rss_ukb_default_roc/susie_rss_ukb_default_pip_init',initial,'_AZ',add_z,'_ld',ldmethod,'_ER',
  ...
  yy = make_smooth(d2$finemapv3$rates$FPR, d2$finemapv3$rates$Recall)
  lines(yy$x, yy$y,col=colors[8], lwd = 2, xlim = c(0,0.0005), ylim = c(0,0.3))
  add_text(d2$finemapv3$rates$Threshold, yy$x, yy$y, 0.95, colors[8], -0.015, print.text=F)
