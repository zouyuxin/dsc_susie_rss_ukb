...
for (case in 1:nrow(all.comb)){
  ...
  input = paste0('susie_rss_ukb_default_pip_extraction/susie_rss_ukb_default_pip_AZ',addz,'_ld',ldmethod,'_ER',
                 estimate_resid, '_lamb', lambda,'.rds')
  output2_name = paste0('susie_rss_ukb_default_roc/susie_rss_ukb_default_pip_AZ',addz,'_ld',ldmethod,'_ER',
                        estimate_resid, '_lamb', lambda, '_roc_2')
  outputall_name = paste0('susie_rss_ukb_default_roc/susie_rss_ukb_default_pip_AZ',addz,'_ld',ldmethod,'_ER',
                          estimate_resid, '_lamb', lambda, '_roc_all')
  ...
  finemapv3 = roc_data(do.call(rbind, lapply(1:3, function(i) cbind(result[[i]]$finemapv3, result[[i]]$is_signal))))
  saveRDS(list(susierss=susierss, susie = susie, finemap=finemap, finemapv3=finemapv3, caviar=caviar), outputall)
  ...
  plot(yy$x, yy$y, t="l", col='black', ylab = "Power", xlab ="False Discovery Rate", main = "1 ~ 5 effect variables",
       bty='l', lwd = 2, xlim = c(0,0.2), ylim = c(0,0.2))
  ...
  lines(yy$x, yy$y, col = colors[5], lwd = 2, xlim = c(0,0.2), ylim = c(0,0.2))
  ...
  yy = make_smooth(1 - d2$finemapv3$rates$Precision, d2$finemapv3$rates$Recall)
  lines(yy$x, yy$y,col=colors[8], lwd = 2, xlim = c(0,0.3), ylim = c(0,0.3))
  add_text(d2$finemapv3$rates$Threshold, yy$x, yy$y, 0.95, colors[8], -0.015)
  ...
  legend("topleft", legend=c("SuSiE", "SuSiE-RSS", "CAVIAR", "FINEMAP", "FINEMAPv3"),
         col=c('black', colors[5], 'green2',colors[7], colors[8]), lty=c(1,1,1,1,1), cex=0.8)
  dev.off()
  ...
  yy = make_smooth(d2$finemap$rates$FPR, d2$finemap$rates$Recall)
  lines(yy$x, yy$y,col=colors[8], lwd = 2, xlim = c(0,0.0005), ylim = c(0,0.3))
  add_text(d2$finemap$rates$Threshold, yy$x, yy$y, 0.95, colors[8], -0.015, print.text=F)
